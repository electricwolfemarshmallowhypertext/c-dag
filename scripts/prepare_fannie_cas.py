"""Normalize Fannie Mae CAS-style public CRT disclosure rows for C-DAG validation.
...
def _normalize_row(row: dict[str, str], index: int) -> dict[str, str]:
    ...
    source_record_id = (
        first_value(row, ("loan_id", "loan_identifier", "loan_sequence_number", "id"))
        or f"cas_{index}"
    )
    mapping_notes = (
        ...
        "segment_proxy_from_pool_property_fields;"
